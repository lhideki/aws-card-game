...
import { challenges, ssmChallenges, allServiceCards, allSupportCards, ssmServiceCards } from '../../../data/index';
...
export default function Game() {
  ...
  const mode = (searchParams.get('mode') === 'ssm' ? 'ssm' : 'default') as 'default' | 'ssm';

  const serviceCards = mode === 'ssm' ? ssmServiceCards : allServiceCards;
  const supportCards = allSupportCards;
  const challengeList = mode === 'ssm' ? ssmChallenges : challenges;
  ...
  useEffect(() => {
    ...
    const initialGameState = initializeGame(serviceCards, supportCards, mode);
    ...
    setTimeout(async () => {
      try {
        ...
        const challenge = selectNewChallenge(challengeList);
